	...
	public void create(){
		...
		add(sp);
		{
			...
			for(int idx=0; idx<CNT_GUIDES; ++idx){
				final int index	=	idx;
				ListItem li	=	new ListItem(Messages.get(this, "title_"+Integer.toString(index))){
					@Override
				...
				li.setRect(0f, index*(HEIGHT_LISTITEM+GAP_LISTITEM), WIDTH_LISTITEM, HEIGHT_LISTITEM);
				content.add(li);
				items_.add(li);
