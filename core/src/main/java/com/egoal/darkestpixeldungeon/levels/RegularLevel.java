...
public abstract class RegularLevel extends Level {
  ...
  protected boolean build() {
    ...
    do {
      ...
      while (roomExit == roomEntrance || roomExit.width() < 4 || roomExit
              .height() < 4);
    ...
    if (Dungeon.shopOnLevel()) {
      ...
      for (Room r : roomEntrance.connected.keySet()) {
        if (r.connected.size() == 1 && ((r.width() - 1) * (r.height() - 1) >=
                ShopPainter.spaceNeeded())) {
          shop = r;
          break;
        }
      }
    ...
    for (Room r : rooms) {
      if (r.type == Type.NULL &&
        ...
        if (specials.size() > 0 &&
          ...
          } else if (Dungeon.depth % 5 == 2 && specials.contains(Type
                  .LABORATORY)) {

            r.type = Type.LABORATORY;

          } else if (Dungeon.depth >= Dungeon.transmutation && specials
                  .contains(Type.MAGIC_WELL)) {
    ...
    for (Room rm : rooms) {
      ...
      if (rm.type == Type.BLACKSMITH || rm.type == Type.ENTRANCE
        ...
        if (Random.Float() < (feeling == Feeling.DARK ? 0.4f : 0.5f)) {
          // have lights
          // int maxLights	=	(rm.type==Type.PASSAGE||rm.type==Type.TUNNEL)? 
          // 3: 2;
        ...
        if (map[pos] == Terrain.WALL && findMob(pos) == null) {
          for (int dp : PathFinder.NEIGHBOURS4) {
            ...
            if (np >= 0 && np < length && r.inside(cellToPoint(np)) &&
                    (map[np] == Terrain.EMPTY || map[np] == Terrain
                            .EMPTY_DECO || map[np] == Terrain.EMPTY_SP)) {
              alCells.add(pos);
              break;
            }
          }
    ...
    for (Room n : r.connected.keySet()) {
      ...
      switch (d.type) {
        ...
        case HIDDEN:
          ...
          break;
        case BARRICADE:
          map[door] = Random.Int(3) == 0 ? Terrain.BOOKSHELF : Terrain
                  .BARRICADE;
          break;
        case LOCKED:
  ...
  public int nMobs() {
    switch (Dungeon.depth) {
      ...
      default:
        ...
        return length > 1200 ? (3 + Dungeon.depth % 5 + Random.Int(6)) :
                (2 + Dungeon.depth % 5 + Random.Int(6));
    }
  }
  ...
  protected void createMobs() {
    ...
    while (mobsToSpawn > 0) {
      ...
      if (findMob(mob.pos) == null && passable[mob.pos]) {
        ...
        //TODO: perhaps externalize this logic into a method. Do I want to 
        // make mobs more likely to clump deeper down?
