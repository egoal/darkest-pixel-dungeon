...
public class LibraryPainter extends Painter {
  ...
  public static void paint(Level level, Room room) {
    ...
    if (entrance.x == room.left) {
      ...
      fill(level, room.right - 1, room.top + 1, 1, room.height() - 1, Terrain
              .BOOKSHELF);
    } else if (entrance.x == room.right) {
      a = new Point(room.right - 1, entrance.y - 1);
      b = new Point(room.right - 1, entrance.y + 1);
      fill(level, room.left + 1, room.top + 1, 1, room.height() - 1, Terrain
              .BOOKSHELF);
      ...
      fill(level, room.left + 1, room.top + 1, room.width() - 1, 1, Terrain
              .BOOKSHELF);
    }
    ...
    for (int i = 0; i < n; i++) {
      ...
      Item item;
      if (i == 0)
        item = Random.Int(2) == 0 ? new ScrollOfIdentify() : new 
                ScrollOfRemoveCurse();
      else
        item = prize(level);
      level.drop(item, pos);
