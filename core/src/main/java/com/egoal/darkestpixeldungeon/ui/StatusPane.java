...
//        clock.setPos(width - clock.width(), danger.bottom() + 4);
    ...
    public void update() {
        ...
        if (Dungeon.INSTANCE.getHero().getSHLD() > 0)
            ...
            hpstr.text(String.format("%d/%d", Dungeon.INSTANCE.getHero().getHP(), Dungeon.INSTANCE.getHero().getHT()));
        hpstr.measure();
        hpstr.x = hp.x + 24f - hpstr.width() / 2f;
    ...
    private static class ClockButton extends Button {
        private Image image;
        Statistics.ClockTime.State lastState = Statistics.ClockTime.State.Day;
        ...
        protected void createChildren() {
            ...
            image = new Image(Assets.DPD_MENU, 0, 16, 12, 11);
            add(image);
        }
        ...
        @Override
        protected void onClick() {
            GameScene.show(new WndTitledMessage(Icons.INFO.get(), M.INSTANCE.L(StatusPane.class, "clock"),
                    M.INSTANCE.L(StatusPane.class, "clock_desc", Statistics.INSTANCE.getClock().getTimestr())));
        }

        @Override
        public void update() {
            if (Statistics.INSTANCE.getClock().getState() != lastState) {
                lastState = Statistics.INSTANCE.getClock().getState();
                int left = 0;
                switch (lastState) {
                    case Day:
                        left = 0;
