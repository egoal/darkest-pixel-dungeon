...
public class Visual extends Gizmo {
  ...
  @Override
  public void draw() {
    // better to use getter& setter to control these behaviour
    if (lastX != x || lastY != y
            || lastW != width || lastH != height || lastA != angle ||
            lastScale.x != scale.x || lastScale.y != scale.y ||
            lastOrigin.x != origin.x || lastOrigin.y != origin.y) {
      lastX = x;
      lastY = y;
      lastW = width;
      lastH = height;
  ...
  public void color(int color) {
    color(((color >> 16) & 0xFF) / 255f, ((color >> 8) & 0xFF) / 255f, (color
            & 0xFF) / 255f);
  }
  ...
  public void hardlight(int color) {
    hardlight((color >> 16) / 255f, ((color >> 8) & 0xFF) / 255f, (color &
            0xFF) / 255f);
  }
  ...
  public boolean overlapsPoint(float x, float y) {
    return x >= this.x && x < this.x + width * scale.x && y >= this.y && y <
            this.y + height * scale.y;
  }
